...
function getComponentSizes() {
  ...
  const files = fs.readdirSync(componentsDir).filter((f) => f.endsWith('.css') && f !== 'index.css');
...
async function main() {
  ...
  console.log('GP-Platform CSS Bundle Size Measurement');
  console.log('='.repeat(60));
  console.log();
  ...
  if (compareOnly && previous) {
    ...
    console.log('Minified (gp.min.css):', formatBytes(previous.minified.bytes).formatted);
    console.log();
    console.log('Component breakdown:');

    const sortedComponents = Object.entries(previous.components).sort((a, b) => b[1].bytes - a[1].bytes);
  ...
  const minifiedPath = path.join(distDir, 'gp.min.css');
  ...
  console.log(
    `Minified (gp.min.css):  ${result.minified.formatted}${previous ? showComparison(minifiedSize, previous.minified.bytes, 'minified') : ''}`
  );
  console.log(
  ...
  for (const [name, size] of sortedComponents.slice(0, 10)) {
    ...
    console.log(`  ${name.padEnd(30)} ${formatBytes(size.bytes).formatted.padStart(12)}${comparison}`);
  }
  ...
  if (previous) {
    ...
    console.log();
    console.log(`Uncompressed change: ${uncompressedDiff > 0 ? '+' : ''}${formatBytes(uncompressedDiff).formatted}`);
    console.log(`Minified change:     ${minifiedDiff > 0 ? '+' : ''}${formatBytes(minifiedDiff).formatted}`);
