...
function processFile(filePath, dryRun = false) {
  ...
  for (const { index, length, replacement, text } of allMatches) {
    ...
    modifiedContent = modifiedContent.substring(0, index) + replacement + modifiedContent.substring(index + length);
  }
  ...
  for (const [lineNumber, lineChange] of lineChanges.entries()) {
    ...
    for (const { length, position, replacement } of lineChange.replacements) {
      modifiedLine = modifiedLine.substring(0, position) + replacement + modifiedLine.substring(position + length);
    }
