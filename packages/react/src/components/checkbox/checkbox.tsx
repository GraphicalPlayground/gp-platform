...
import type { CheckboxVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode } from 'react';
import type { CheckboxRenderProps } from 'react-aria-components/Checkbox';

import { checkboxVariants } from '@gp/styles';
...
const CheckboxRoot = ({ children, className, variant, ...props }: CheckboxRootProps) => {
  ...
  const slots = React.useMemo(() => checkboxVariants({ variant: effectiveVariant }), [effectiveVariant]);

  return (
    <CheckboxPrimitive data-slot='checkbox' {...props} className={composeTwRenderProps(className, slots.base())}>
      {(values) => (
...
interface CheckboxControlProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const CheckboxControl = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  className,
  ...props
}: CheckboxControlProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof CheckboxControlProps<E>>) => {
...
interface CheckboxIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
...
const CheckboxIndicator = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  className,
  ...props
}: CheckboxIndicatorProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof CheckboxIndicatorProps<E>>) => {
...
interface CheckboxContentProps<E extends keyof React.JSX.IntrinsicElements = 'div'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const CheckboxContent = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  className,
  ...props
}: CheckboxContentProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof CheckboxContentProps<E>>) => {
  const { slots } = useContext(CheckboxContext);
