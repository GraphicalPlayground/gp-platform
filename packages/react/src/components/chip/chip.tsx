...
import type { ChipVariants } from '@gp/styles';
import type { ReactNode } from 'react';

import { chipVariants } from '@gp/styles';
...
const ChipRoot = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <ChipContext value={{ slots }}>
      <dom.span {...(props as any)} className={composeSlotClassName(slots.base, className)} data-slot='chip'>
        {chipChildren}
      </dom.span>
    </ChipContext>
  );
...
const ChipLabel = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span className={composeSlotClassName(slots?.label, className)} data-slot='chip-label' {...(props as any)}>
      {children}
    </dom.span>
  );
};
