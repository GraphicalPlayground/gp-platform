...
import type { MenuItemVariants } from '@gp/styles';
import type { ComponentPropsWithRef } from 'react';
import type { MenuItemRenderProps } from 'react-aria-components/Menu';

import { menuItemVariants } from '@gp/styles';
...
const MenuItemRoot = ({ children, className, variant, ...props }: MenuItemRootProps) => {
  ...
  return (
    <MenuItemPrimitive className={composeTwRenderProps(className, slots.item())} data-slot='menu-item' {...props}>
      {(values) => (
 ...
 * Menu Item Indicator
 * -----------------------------------------------------------------------------------------------*/
interface MenuItemIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
...
const MenuItemIndicator = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  type = 'checkmark',
  ...props
}: MenuItemIndicatorProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof MenuItemIndicatorProps<E>>) => {
 ...
 * -----------------------------------------------------------------------------------------------*/
interface MenuItemSubmenuIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
...
const MenuItemSubmenuIndicator = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  className,
  ...props
}: MenuItemSubmenuIndicatorProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof MenuItemSubmenuIndicatorProps<E>>) => {
  const { slots, state } = useContext(MenuItemContext);
