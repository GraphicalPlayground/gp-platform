...
import type { InputOTPVariants } from '@gp/styles';
import type { ComponentPropsWithRef } from 'react';
import type { ValidationResult } from 'react-aria-components/CheckboxGroup';

import { inputOTPVariants } from '@gp/styles';
...
interface InputOTPRootProps
  extends Omit<ComponentPropsWithRef<typeof OTPInput>, 'disabled' | 'containerClassName' | 'render'>, InputOTPVariants {
  isDisabled?: boolean;
  isInvalid?: boolean;
...
const InputOTPGroup = ({ className, ...props }: InputOTPGroupProps) => {
  ...
  return <div className={composeSlotClassName(slots?.group, className)} data-slot='input-otp-group' {...props} />;
};
...
const InputOTPSlot = ({ className, index, ...props }: InputOTPSlotProps) => {
  ...
  return (
    ...
    >
      ...
      {hasFakeCaret && isActive ? <div className={slots?.caret()} data-slot='input-otp-caret' /> : null}
    </div>
  );
};
...
const InputOTPSeparator = ({ className, ...props }: InputOTPSeparatorProps) => {
  ...
  return (
    <div className={composeSlotClassName(slots?.separator, className)} data-slot='input-otp-separator' {...props} />
  );
};
