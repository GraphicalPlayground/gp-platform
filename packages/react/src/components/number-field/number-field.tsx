...
import type { NumberFieldVariants } from '@gp/styles';
import type { ComponentPropsWithRef } from 'react';

import { numberFieldVariants } from '@gp/styles';
...
const NumberFieldRoot = ({ children, className, fullWidth, variant, ...props }: NumberFieldRootProps) => {
  const slots = React.useMemo(() => numberFieldVariants({ fullWidth, variant }), [fullWidth, variant]);
...
const NumberFieldIncrementButton = ({ children, className, ...props }: NumberFieldIncrementButtonProps) => {
  const { slots } = useContext(NumberFieldContext);
...
const NumberFieldDecrementButton = ({ children, className, ...props }: NumberFieldDecrementButtonProps) => {
  const { slots } = useContext(NumberFieldContext);
