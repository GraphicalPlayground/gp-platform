...
export const SelectionModes: Story = {
  render: () => {
    ...
    const [multipleSelected, setMultipleSelected] = useState<Iterable<Key>>(new Set(['news', 'travel']));
  ...
  render: () => {
    ...
    return (
      <div className='flex flex-col gap-3'>
        <TagGroup selectedKeys={selected} selectionMode='multiple' onSelectionChange={(keys) => setSelected(keys)}>
  ...
  render: () => {
    ...
    return (
      <TagGroup selectedKeys={selected} selectionMode='multiple' onSelectionChange={(keys) => setSelected(keys)}>
        ...
        <Description>
          {isInvalid ? 'Select at least one category' : 'Selected: ' + Array.from(selected).join(', ')}
        </Description>
