...
import type { SwitchVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode } from 'react';

import { switchVariants } from '@gp/styles';
...
const SwitchRoot = ({ children, className, size, ...props }: SwitchRootProps) => {
  ...
  return (
    <SwitchContext value={{ slots }}>
      <SwitchPrimitive data-slot='switch' {...props} className={composeTwRenderProps(className, slots.base())}>
 ...
 * Switch Control
 * -----------------------------------------------------------------------------------------------*/
interface SwitchControlProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const SwitchThumb = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span className={composeSlotClassName(slots?.thumb, className)} data-slot='switch-thumb' {...(props as any)}>
      {children}
    </dom.span>
  );
};
...
const SwitchIcon = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span className={composeSlotClassName(slots?.icon, className)} data-slot='switch-icon' {...(props as any)}>
      {children}
    </dom.span>
  );
};
...
const SwitchContent = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  return (
    <dom.div className={composeSlotClassName(slots?.content, className)} data-slot='switch-content' {...(props as any)}>
      {children}
    </dom.div>
  );
};
