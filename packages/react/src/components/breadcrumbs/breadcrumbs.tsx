...
import type { BreadcrumbsVariants } from '@gp/styles';
import type { ComponentPropsWithRef } from 'react';

import { breadcrumbsVariants } from '@gp/styles';
...
const BreadcrumbsRoot = ({ children, className, separator, ...props }: BreadcrumbsRootProps) => {
  ...
  return (
    <BreadcrumbsContext.Provider value={{ slots, separator }}>
      <BreadcrumbsPrimitive data-slot='breadcrumbs' {...props} className={composeSlotClassName(slots.base, className)}>
        {children}
...
const BreadcrumbsItem = ({ children, className, ...props }: BreadcrumbsItemProps & Omit<LinkProps, 'className'>) => {
  const { separator, slots } = useContext(BreadcrumbsContext);

  const renderSeparator = () => {
    if (!separator) return <IconChevronRight className={slots?.separator()} data-slot='breadcrumbs-separator' />;
