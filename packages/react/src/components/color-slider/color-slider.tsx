...
function getValidColorSpace(channel: string, colorSpace?: ColorSpace): ColorSpace | undefined {
  ...
  if (requiredSpace && colorSpace && colorSpace !== requiredSpace) {
    ...
    console.warn(
      `[GPlayd ColorSlider] Invalid combination: channel="${channel}" requires colorSpace="${requiredSpace}", ` +
  ...
  if (HSL_HSB_ONLY_CHANNELS.has(channel) && colorSpace === 'rgb') {
    // eslint-disable-next-line no-console
    console.warn(
      `[GPlayd ColorSlider] Invalid combination: channel="${channel}" is not available in RGB color space. ` +
...
interface ColorSliderRootBaseProps
  extends Omit<ComponentPropsWithRef<typeof ColorSliderPrimitive>, 'channel' | 'colorSpace'>, ColorSliderVariants {}
...
const ColorSliderTrack = ({ children, className, style, ...props }: ColorSliderTrackProps) => {
  ...
  const edgeColors = React.useMemo(() => {
    ...
    const range = displayColor.getChannelRange(channel as Parameters<typeof displayColor.getChannelRange>[0]);
