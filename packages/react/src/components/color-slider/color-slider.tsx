...
function getValidColorSpace(channel: string, colorSpace?: ColorSpace): ColorSpace | undefined {
  ...
  if (requiredSpace && colorSpace && colorSpace !== requiredSpace) {
    ...
    console.warn(
      `[GP ColorSlider] Invalid combination: channel="${channel}" requires colorSpace="${requiredSpace}", ` +
  ...
  if (HSL_HSB_ONLY_CHANNELS.has(channel) && colorSpace === 'rgb') {
    // eslint-disable-next-line no-console
    console.warn(
      `[GP ColorSlider] Invalid combination: channel="${channel}" is not available in RGB color space. ` +
