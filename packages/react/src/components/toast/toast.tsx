...
import type { ToastVariants } from '@gp/styles';
...
import { toastVariants } from '@gp/styles';
...
Toast.displayName = 'GPlayd.Toast';
 ...
 * Toast Indicator
 * --------------------------------------------------------------------------------------------- */
interface ToastIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'div'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
ToastIndicator.displayName = 'GPlayd.ToastIndicator';
...
ToastTitle.displayName = 'GPlayd.ToastTitle';
...
ToastDescription.displayName = 'GPlayd.ToastDescription';
...
ToastCloseButton.displayName = 'GPlayd.ToastCloseButton';
...
const ToastActionButton = ({ children, className, ...rest }: ToastActionButtonProps) => {
  ...
  return (
    <Button className={composeTwRenderProps(className, slots?.action?.())} data-slot='toast-action-button' {...rest}>
      {children}
    </Button>
  );
};

ToastActionButton.displayName = 'GPlayd.ToastActionButton';
...
const ToastProvider = <T extends object = ToastContentValue>({
  ...
  const toastQueue = useMemo((): StatelyToastQueue<T> => {
    if (queueProp) {
      // Region consumes the underlying react-stately queue, not the GPlayd wrapper.
  ...
  const getDefaultChildren = useCallback(
    (renderProps: { toast: QueuedToast<T> }) => {
      ...
      return (
        <Toast placement={placement} scaleFactor={scaleFactor} toast={renderProps.toast} variant={variant}>
...
ToastProvider.displayName = 'GPlayd.ToastProvider';
