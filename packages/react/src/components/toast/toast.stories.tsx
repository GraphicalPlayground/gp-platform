...
import type { GPlaydToastOptions } from './toast-queue';
...
import { Toast, ToastContent, ToastDescription, ToastIndicator, ToastQueue, ToastTitle, toast } from './index';

type Placement = NonNullable<ToastVariants['placement']>;

interface ToastStoryProps extends Omit<GPlaydToastOptions, 'variant'> {
  placement?: Placement;
}
        ...
        <Button
          ...
          onPress={() => {
            toast('You have been invited to join a team', {
              ...
              description: 'Bob sent you an invitation to join GPlayd team',
        ...
        <Button
          ...
          onPress={() =>
            toast.success('You have upgraded your plan', {
              ...
              description: 'You can continue using GPlayd Chat'
            })
          }
        >
          Success toast
      ...
      <div className='w-full space-y-2'>
        <div className='flex items-center justify-between'>
          ...
          {closedHistory.length > 0 && (
            <Button className='h-6 text-xs' size='sm' variant='tertiary' onPress={() => setClosedHistory([])}>
              Clear
            </Button>
          )}
        </div>
    ...
    <div className='flex h-full max-w-xl flex-col items-center justify-center'>
      <Toast.Provider placement='bottom' queue={customQueue}>
        {({ toast: toastItem }) => {
          ...
          return (
            <Toast className='rounded-xl border border-border' toast={toastItem} variant={content.variant}>
              <ToastContent>
                <div className='flex items-center gap-2'>
                  <ToastIndicator className='text-accent' variant={content.variant} />
                  <div className='flex flex-col pr-6'>
                    {content.title ? <ToastTitle className='text-accent'>{content.title}</ToastTitle> : null}
