...
export interface GPToastOptions {
...
function createToastFunction(queue: ToastQueue<ToastContentValue>) {
  const toastFn = (message: ReactNode, options?: GPToastOptions): string => {
  ...
  // Variant methods
  toastFn.success = (message: ReactNode, options?: Omit<GPToastOptions, 'variant'>): string => {
    return toastFn(message, { ...options, variant: 'success' });
  };

  toastFn.danger = (message: ReactNode, options?: Omit<GPToastOptions, 'variant'>): string => {
    return toastFn(message, { ...options, variant: 'danger' });
  };

  toastFn.info = (message: ReactNode, options?: Omit<GPToastOptions, 'variant'>): string => {
    return toastFn(message, { ...options, variant: 'accent' });
  };

  toastFn.warning = (message: ReactNode, options?: Omit<GPToastOptions, 'variant'>): string => {
