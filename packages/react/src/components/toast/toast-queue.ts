 ...
 * Toast Queue
 * --------------------------------------------------------------------------------------------- */
/** The underlying react-stately queue passed to `ToastRegion` (not the GPlayd `ToastQueue` wrapper). */
...
export interface GPlaydToastOptions {
...
function createToastFunction(queue: ToastQueue<ToastContentValue>) {
  const toastFn = (message: ReactNode, options?: GPlaydToastOptions): string => {
  ...
  // Variant methods
  toastFn.success = (message: ReactNode, options?: Omit<GPlaydToastOptions, 'variant'>): string => {
    return toastFn(message, { ...options, variant: 'success' });
  };

  toastFn.danger = (message: ReactNode, options?: Omit<GPlaydToastOptions, 'variant'>): string => {
    return toastFn(message, { ...options, variant: 'danger' });
  };

  toastFn.info = (message: ReactNode, options?: Omit<GPlaydToastOptions, 'variant'>): string => {
    return toastFn(message, { ...options, variant: 'accent' });
  };

  toastFn.warning = (message: ReactNode, options?: Omit<GPlaydToastOptions, 'variant'>): string => {
