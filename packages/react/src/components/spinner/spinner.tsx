...
import type { SpinnerVariants } from '@gp/styles';
import type { ComponentPropsWithRef } from 'react';

import { spinnerVariants } from '@gp/styles';
...
const SpinnerPrimitive = ({ ...props }: SpinnerPrimitiveProps) => {
  ...
  return (
    <svg data-slot='spinner-icon' viewBox='0 0 24 24' {...props}>
      <defs>
        <linearGradient id={`«data-slot-icon-def-1»-${id}`} x1='50%' x2='50%' y1='5.271%' y2='91.793%'>
          <stop offset='0%' stopColor='currentColor' />
          <stop offset='100%' stopColor='currentColor' stopOpacity={0.55} />
        </linearGradient>
        <linearGradient id={`«data-slot-icon-def-2»-${id}`} x1='50%' x2='50%' y1='15.24%' y2='87.15%'>
