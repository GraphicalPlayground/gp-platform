...
import type { DisclosureVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode } from 'react';
import type { ButtonProps } from 'react-aria-components/Button';

import { disclosureVariants } from '@gp/styles';
...
const DisclosureRoot = ({ children, className, ...props }: DisclosureRootProps) => {
  ...
  return (
    <DisclosureContext value={{ slots }}>
      <DisclosurePrimitive data-slot='disclosure' {...props} className={composeTwRenderProps(className, slots.base())}>
 ...
 * Disclosure Body
 * -----------------------------------------------------------------------------------------------*/
interface DisclosureBodyContentProps<E extends keyof React.JSX.IntrinsicElements = 'div'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const DisclosureBody = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  className,
  ...props
}: DisclosureBodyContentProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof DisclosureBodyContentProps<E>>) => {
  const { slots } = useContext(DisclosureContext);
 ...
 * -----------------------------------------------------------------------------------------------*/
interface DisclosureIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'svg'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const DisclosureIndicator = <E extends keyof React.JSX.IntrinsicElements = 'svg'>({
  ...
  className,
  ...props
}: DisclosureIndicatorProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof DisclosureIndicatorProps<E>>) => {
  const { isExpanded } = useContext(DisclosureStateContext)!;
  const { slots } = useContext(DisclosureContext);
