...
import type { RangeCalendarVariants } from '@gp/styles';
import type { CalendarIdentifier } from '@internationalized/date';
import type { ComponentPropsWithRef, ReactNode } from 'react';
import type { DateValue } from 'react-aria-components/Calendar';

import { rangeCalendarVariants } from '@gp/styles';
...
import { YearPickerContext, YearPickerStateContext } from '../calendar-year-picker/year-picker-context';
...
function RangeCalendarRoot<T extends DateValue = DateValue>({
  ...
  const calendarProp = React.useMemo(() => {
    const calendarIdentifier = new DateFormatter(locale).resolvedOptions().calendar as CalendarIdentifier;
  ...
  const minValue = minValueProp ?? (new CalendarDate(calendarProp, 1900 + gregorianYearOffset, 1, 1) as unknown as T);
  const maxValue = maxValueProp ?? (new CalendarDate(calendarProp, 2099 + gregorianYearOffset, 12, 31) as unknown as T);
...
RangeCalendarRoot.displayName = 'GPlayd.RangeCalendar';

/* -------------------------------------------------------------------------------------------------
| * RangeCalendar Header
| * -----------------------------------------------------------------------------------------------*/
interface RangeCalendarHeaderProps<E extends keyof React.JSX.IntrinsicElements = 'header'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const RangeCalendarHeader = <E extends keyof React.JSX.IntrinsicElements = 'header'>({
  ...
  className,
  ...props
}: RangeCalendarHeaderProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof RangeCalendarHeaderProps<E>>) => {
...
RangeCalendarHeader.displayName = 'GPlayd.RangeCalendar.Header';
...
RangeCalendarHeading.displayName = 'GPlayd.RangeCalendar.Heading';
...
const RangeCalendarNavButton = ({ children, className, slot, ...props }: RangeCalendarNavButtonProps) => {
  const { slots } = useContext(RangeCalendarContext);
  ...
  return (
    ...
    >
      {children ||
        (slot === 'previous' ? (
          <IconChevronLeft className={slots?.navButtonIcon()} data-slot='range-calendar-nav-button-icon' />
        ) : (
          <IconChevronRight className={slots?.navButtonIcon()} data-slot='range-calendar-nav-button-icon' />
        ))}
    </ButtonPrimitive>
  );
};

RangeCalendarNavButton.displayName = 'GPlayd.RangeCalendar.NavButton';
...
const RangeCalendarGrid = ({ children, className, weekdayStyle = 'short', ...props }: RangeCalendarGridProps) => {
  const { slots } = useContext(RangeCalendarContext);
...
RangeCalendarGrid.displayName = 'GPlayd.RangeCalendar.Grid';
...
RangeCalendarGridHeader.displayName = 'GPlayd.RangeCalendar.GridHeader';
...
RangeCalendarGridBody.displayName = 'GPlayd.RangeCalendar.GridBody';
...
RangeCalendarHeaderCell.displayName = 'GPlayd.RangeCalendar.HeaderCell';
...
const RangeCalendarCell = ({ children, className, ...props }: RangeCalendarCellProps) => {
  ...
  return (
    ...
    >
      {(values) => {
        const { formattedDate, isDisabled, isHovered, isPressed, isSelectionEnd, isSelectionStart } = values;
...
RangeCalendarCell.displayName = 'GPlayd.RangeCalendar.Cell';

/* -------------------------------------------------------------------------------------------------
| * RangeCalendar Cell Indicator
| * -----------------------------------------------------------------------------------------------*/
interface RangeCalendarCellIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
  className?: string;
...
RangeCalendarCellIndicator.displayName = 'GPlayd.RangeCalendar.CellIndicator';
