    ...
    <Accordion {...props}>
      {items.map((item, index) => (
        <Accordion.Item key={index}>
          <Accordion.Heading>
            <Accordion.Trigger>
              {item.icon ? <Icon className='mr-3 size-4 shrink-0 text-muted' icon={item.icon} /> : null}
              {item.title}
              <Accordion.Indicator>
  ...
  <div className='flex w-full justify-center px-4 py-8'>
    <div className='w-full max-w-2xl'>
      <div className='flex flex-col gap-1'>
        ...
        <p className='mb-4 text-lg font-medium text-muted'>Everything you need to know about licensing and usage.</p>
      </div>
...
const categories = [
  ...
  },
  {
    ...
    items: [
      {
        title: 'How do I use the GPlayd icon set?',
        content:
          "Browse our products, add items to your cart, and proceed to checkout. You'll need to provide shipping and payment information to complete your purchase."
      },
      {
