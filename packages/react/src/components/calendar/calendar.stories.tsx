 ...
 * -----------------------------------------------------------------------------------------------*/
const CalendarTemplateWithYearPicker = (props: Omit<React.ComponentProps<typeof Calendar>, 'children'>) => (
  <Calendar {...props}>
    <Calendar.Header>
...
const CalendarMonthHeading = ({ offset = 0 }: { offset?: number }) => {
  ...
  const monthYear = new Intl.DateTimeFormat(locale, { month: 'long', year: 'numeric' }).format(dateObj);
...
export const DefaultValue: Story = {
  render: (args) => <CalendarTemplate {...args} aria-label='Event date' defaultValue={parseDate('2025-02-14')} />
};
  ...
  render: (args) => {
    ...
    return (
      <div className='flex flex-col items-center gap-4'>
        ...
        />
        <Description className='text-center'>Selected date: {value ? value.toString() : '(none)'}</Description>
  ...
  render: (args) => {
    ...
    return (
      <div className='flex flex-col items-center gap-4'>
        ...
        <Description className='text-center'>Select a date between today and {maxDate.toString()}</Description>
      </div>
    );
  }
};
...
export const UnavailableDates: Story = {
  render: (args) => {
    ...
    return (
      <div className='flex flex-col items-center gap-4'>
        <CalendarTemplate {...args} aria-label='Appointment date' isDateUnavailable={isDateUnavailable} />
  ...
  render: (args) => {
    ...
    const isDateUnavailable = (date: DateValue) => {
      return blockedDates.some(
        (blockedDate) =>
          blockedDate.year === date.year && blockedDate.month === date.month && blockedDate.day === date.day
      );
    };
    ...
    return (
      <div className='flex flex-col items-center gap-4'>
        ...
        <Description className='text-center'>Feb 14, Feb 17, and Mar 17 are unavailable</Description>
      </div>
    );
  }
};
...
export const Disabled: Story = {
  render: (args) => (
    <div className='flex flex-col items-center gap-4'>
      <CalendarTemplate {...args} isDisabled aria-label='Event date' defaultValue={today(getLocalTimeZone())} />
...
export const ReadOnly: Story = {
  render: (args) => (
    <div className='flex flex-col items-center gap-4'>
      <CalendarTemplate {...args} isReadOnly aria-label='Event date' defaultValue={today(getLocalTimeZone())} />
...
export const Invalid: Story = {
  render: (args) => {
    ...
    return (
      <div className='flex flex-col items-center gap-4'>
        <CalendarTemplate {...args} aria-label='Event date' isInvalid={isInvalid} value={value} onChange={setValue} />
        {isInvalid ? (
...
export const FocusedValue: Story = {
  render: (args) => {
    ...
    return (
      <div className='flex flex-col items-center gap-4'>
        <CalendarTemplate {...args} aria-label='Event date' focusedValue={focusedDate} onFocusChange={setFocusedDate} />
        <Description className='text-center'>Focused: {focusedDate.toString()}</Description>
        <div className='flex flex-wrap justify-center gap-2'>
          <Button size='sm' variant='secondary' onPress={() => setFocusedDate(parseDate('2025-01-01'))}>
            Go to Jan
          </Button>
          <Button size='sm' variant='secondary' onPress={() => setFocusedDate(parseDate('2025-06-15'))}>
            Go to Jun
          </Button>
          <Button size='sm' variant='secondary' onPress={() => setFocusedDate(parseDate('2025-12-25'))}>
            Go to Christmas
          </Button>
        </div>
      </div>
...
export const MultipleMonths: Story = {
  render: (args) => (
    <Calendar {...args} aria-label='Trip dates' className='@container-normal w-auto' visibleDuration={{ months: 2 }}>
...
export const InternationalCalendar: Story = {
  render: (args) => (
    <I18nProvider locale='hi-IN-u-ca-indian'>
      <CalendarTemplateWithYearPicker {...args} aria-label='Event date' defaultValue={today(getLocalTimeZone())} />
    </I18nProvider>
  )
};
        ...
        <Calendar.YearPickerGridBody>
          {({ isCurrentYear, isSelected, year }) => (
            <Calendar.YearPickerCell
              className={isCurrentYear && !isSelected ? 'text-accent ring-1 ring-accent/60 ring-inset' : undefined}
              year={year}
            />
          )}
        </Calendar.YearPickerGridBody>
