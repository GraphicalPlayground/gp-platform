...
import type { SelectVariants } from '@gp/styles';
import type { ComponentPropsWithRef } from 'react';

import { selectVariants } from '@gp/styles';
...
const SelectRoot = <T extends object = object, M extends 'single' | 'multiple' = 'single'>({
  ...
  return (
    <SelectContext value={{ slots }}>
      <SelectPrimitive data-slot='select' {...props} className={composeTwRenderProps(className, slots?.base())}>
 ...
 * Select Indicator
 * -----------------------------------------------------------------------------------------------*/
interface SelectIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'svg'> extends DOMRenderProps<
  E,
  undefined
> {
...
const SelectIndicator = <E extends keyof React.JSX.IntrinsicElements = 'svg'>({
  ...
  className,
  ...props
}: SelectIndicatorProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof SelectIndicatorProps<E>>) => {
  const { slots } = useContext(SelectContext);
...
const SelectPopover = ({ children, className, placement = 'bottom', ...props }: SelectPopoverProps) => {
  const { slots } = useContext(SelectContext);
