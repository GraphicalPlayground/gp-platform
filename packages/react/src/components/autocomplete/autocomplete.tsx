...
import type { AutocompleteVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode, RefObject } from 'react';

import { autocompleteVariants } from '@gp/styles';
...
const AutocompleteRoot = <T extends object = object, M extends 'single' | 'multiple' = 'single'>({
  ...
  const slots = React.useMemo(() => autocompleteVariants({ fullWidth, variant }), [fullWidth, variant]);
...
const AutocompleteTrigger = React.forwardRef<HTMLDivElement, AutocompleteTriggerProps>(
  ({ children, className, isDisabled: isDisabledProp, onClick, ...props }, ref) => {
    const { clearButtonRef, isDisabled: rootDisabled, slots, triggerRef } = useContext(AutocompleteContext);
 ...
 * -----------------------------------------------------------------------------------------------*/
interface AutocompleteIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'svg'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const AutocompleteIndicator = <E extends keyof React.JSX.IntrinsicElements = 'svg'>({
  ...
  className,
  ...props
}: AutocompleteIndicatorProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof AutocompleteIndicatorProps<E>>) => {
  const { slots } = useContext(AutocompleteContext);
  const state = useContext(SelectStateContext);
 ...
 * -----------------------------------------------------------------------------------------------*/
interface AutocompleteClearButtonProps<E extends keyof React.JSX.IntrinsicElements = 'button'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const AutocompleteClearButton = <E extends keyof React.JSX.IntrinsicElements = 'button'>({
  ...
  ref,
  ...props
}: AutocompleteClearButtonProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof AutocompleteClearButtonProps<E>>) => {
