...
const AutocompleteRoot = <T extends object = object, M extends 'single' | 'multiple' = 'single'>({
  ...
  return (
    <AutocompleteContext value={{ slots, triggerRef, clearButtonRef, onClear }}>
      <SelectPrimitive data-slot='autocomplete' {...props} className={composeTwRenderProps(className, slots?.base())}>
...
const AutocompletePopover = ({ children, className, placement = 'bottom', ...props }: AutocompletePopoverProps) => {
...
const AutocompleteClearButton = ({ className, onClick, ref, ...props }: AutocompleteClearButtonProps) => {
  const { slots } = useContext(AutocompleteContext);
  const state = useContext(SelectStateContext);
