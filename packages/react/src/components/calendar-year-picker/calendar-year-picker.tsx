...
import type { CalendarYearPickerVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode } from 'react';

import { calendarYearPickerVariants } from '@gp/styles';
...
interface CalendarYearPickerTriggerProps
  extends Omit<ComponentPropsWithRef<typeof ButtonPrimitive>, 'children'>, CalendarYearPickerVariants {
...
interface CalendarYearPickerTriggerHeadingProps<E extends keyof React.JSX.IntrinsicElements = 'span'>
...
interface CalendarYearPickerTriggerIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'span'>
  extends DOMRenderProps<E, undefined>, CalendarYearPickerVariants {
  children?: ReactNode | ((values: CalendarYearPickerTriggerRenderProps) => ReactNode);
  className?: string;
}
...
const CalendarYearPickerTriggerContext = React.createContext<CalendarYearPickerTriggerContextValue | null>(null);
...
const CalendarYearPickerTrigger = ({
  ...
  const handleKeyDown = (e: Parameters<NonNullable<CalendarYearPickerTriggerProps['onKeyDown']>>[0]) => {
    onKeyDown?.(e);
...
CalendarYearPickerTrigger.displayName = 'GPlayd.CalendarYearPicker.Trigger';
...
CalendarYearPickerTriggerHeading.displayName = 'GPlayd.CalendarYearPicker.TriggerHeading';
...
CalendarYearPickerTriggerIndicator.displayName = 'GPlayd.CalendarYearPicker.TriggerIndicator';
...
const CalendarYearPickerGridContext = React.createContext<CalendarYearPickerGridContextValue | null>(null);
...
const CalendarYearPickerGrid = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  onKeyDown,
  ...props
}: CalendarYearPickerGridProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof CalendarYearPickerGridProps<E>>) => {
  ...
  const yearDates = React.useMemo(() => getYearRange(state.minValue, state.maxValue), [state.minValue, state.maxValue]);
  ...
  React.useEffect(() => {
    ...
    const calendarGrid = calendar?.querySelector(`[data-slot='${calendarGridSlot}']`) as HTMLElement | null;
...
CalendarYearPickerGrid.displayName = 'GPlayd.CalendarYearPicker.Grid';

/* -------------------------------------------------------------------------------------------------
 * CalendarYearPickerGridBody
 * -----------------------------------------------------------------------------------------------*/
const CalendarYearPickerGridBody = ({ children }: CalendarYearPickerGridBodyProps) => {
  const { focusedYear, formatYear, isYearPickerOpen, selectYear, years } = useCalendarYearPickerGridContext();
      ...
      {years.map((year) => {
        ...
        return (
          <React.Fragment key={year}>
            {typeof children === 'function' ? children(values) : <CalendarYearPickerCell year={year} />}
          </React.Fragment>
        );
      })}
    </>
...
CalendarYearPickerGridBody.displayName = 'GPlayd.CalendarYearPicker.GridBody';
...
CalendarYearPickerCell.displayName = 'GPlayd.CalendarYearPicker.Cell';
