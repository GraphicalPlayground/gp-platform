...
import type { ColorInputGroupVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode } from 'react';
import type { InputProps as InputPrimitiveProps } from 'react-aria-components/Input';

import { colorInputGroupVariants } from '@gp/styles';
...
const ColorInputGroupRoot = ({ children, className, fullWidth, variant, ...props }: ColorInputGroupRootProps) => {
  const slots = React.useMemo(() => colorInputGroupVariants({ fullWidth, variant }), [fullWidth, variant]);
 ...
 * -----------------------------------------------------------------------------------------------*/
interface ColorInputGroupPrefixProps<E extends keyof React.JSX.IntrinsicElements = 'div'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const ColorInputGroupPrefix = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  className,
  ...props
}: ColorInputGroupPrefixProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof ColorInputGroupPrefixProps<E>>) => {
 ...
 * -----------------------------------------------------------------------------------------------*/
interface ColorInputGroupSuffixProps<E extends keyof React.JSX.IntrinsicElements = 'div'> extends DOMRenderProps<
  E,
  undefined
> {
  children?: ReactNode;
...
const ColorInputGroupSuffix = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  className,
  ...props
}: ColorInputGroupSuffixProps<E> & Omit<React.JSX.IntrinsicElements[E], keyof ColorInputGroupSuffixProps<E>>) => {
  const { slots } = useContext(ColorInputGroupContext);
