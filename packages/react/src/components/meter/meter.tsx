...
import type { MeterVariants } from '@gp/styles';
import type { ComponentPropsWithRef, ReactNode } from 'react';
import type { MeterRenderProps } from 'react-aria-components/Meter';

import { meterVariants } from '@gp/styles';
...
const MeterRoot = ({ children, className, color, size, ...props }: MeterRootProps) => {
  ...
  return (
    <MeterPrimitive data-slot='meter' {...props} className={composeTwRenderProps(className, slots.base())}>
      {(values) => (
...
MeterRoot.displayName = 'GPlayd.Meter';
...
const MeterOutput = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span className={composeSlotClassName(slots?.output, className)} data-slot='meter-output' {...(props as any)}>
      {children ?? state?.valueText}
    </dom.span>
  );
};

MeterOutput.displayName = 'GPlayd.Meter.Output';
...
const MeterTrack = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  return (
    <dom.div className={composeSlotClassName(slots?.track, className)} data-slot='meter-track' {...(props as any)}>
      {children}
    </dom.div>
  );
};

MeterTrack.displayName = 'GPlayd.Meter.Track';
...
MeterFill.displayName = 'GPlayd.Meter.Fill';
