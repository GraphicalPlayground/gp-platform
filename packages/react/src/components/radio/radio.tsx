...
import { radioVariants } from '@gp/styles';
...
const RadioRoot = ({ children, className, ...props }: RadioRootProps) => {
  ...
  return (
    <RadioPrimitive data-slot='radio' {...props} className={composeTwRenderProps(className, slots.base())}>
      {(values) => (
...
const RadioControl = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span className={composeSlotClassName(slots?.control, className)} data-slot='radio-control' {...(props as any)}>
      {children}
    </dom.span>
  );
};
 ...
 * Radio Indicator
 * -----------------------------------------------------------------------------------------------*/
interface RadioIndicatorProps<E extends keyof React.JSX.IntrinsicElements = 'span'> extends DOMRenderProps<
  E,
  undefined
> {
...
const RadioIndicator = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  const content = typeof children === 'function' ? children(state ?? ({} as RadioRenderProps)) : children;
...
const RadioContent = <E extends keyof React.JSX.IntrinsicElements = 'div'>({
  ...
  return (
    <dom.div className={composeSlotClassName(slots?.content, className)} data-slot='radio-content' {...(props as any)}>
      {children}
    </dom.div>
  );
};
