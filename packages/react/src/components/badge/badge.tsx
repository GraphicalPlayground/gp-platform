...
import type { BadgeVariants } from '@gp/styles';
import type { ReactNode } from 'react';

import { badgeVariants } from '@gp/styles';
...
const BadgeAnchor = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span {...(props as any)} className={cx('badge-anchor', className) ?? undefined} data-slot='badge-anchor'>
      {children}
    </dom.span>
  );
};
...
const BadgeRoot = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <BadgeContext value={{ slots }}>
      <dom.span {...(props as any)} className={composeSlotClassName(slots.base, className)} data-slot='badge'>
        {badgeChildren}
      </dom.span>
    </BadgeContext>
  );
...
const BadgeLabel = <E extends keyof React.JSX.IntrinsicElements = 'span'>({
  ...
  return (
    <dom.span className={composeSlotClassName(slots?.label, className)} data-slot='badge-label' {...(props as any)}>
      {children}
    </dom.span>
  );
};
