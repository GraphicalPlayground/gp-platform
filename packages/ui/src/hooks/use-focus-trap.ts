...
> {
  /**
   * @brief Ref object that will be used for the trapping container. If not provided, one will
   * be created by this hook and returned.
   */
  containerRef?: React.RefObject<ContainerElement | null>;

  /**
   * @brief Ref object for the element that should receive focus when the focus trap is first enabled.
  ...
  /**
   * @brief Set to true to disable the focus trap and clean up listeners. Can be re-enabled at any time.
   */
  disabled?: boolean;

  /**
   * @brief If true, when this focus trap is cleaned up, restore focus to the element that had focus immediately before the focus trap was enabled. (Default: false)
   */
  restoreFocusOnCleanUp?: boolean;
}

/**
 * @brief Hook used to trap focus inside a container. Returns a ref that can be added to the container
