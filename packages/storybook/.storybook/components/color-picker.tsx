import type { ColorPickerProps as AriaColorPickerProps, Color, ColorSpace } from 'react-aria-components';

import { ChevronLeft, ChevronRight, Shuffle } from '@gravity-ui/icons';
import { Button, InputGroup, ListBox, Popover, Select, parseColor } from '@gp/react';
...
function ColorSlider({ channel, className, colorSpace, orientation = 'horizontal' }: ColorSliderProps) {
  return (
    <AriaColorSlider
      channel={channel}
...
function ColorField({ channel, className, colorSpace, label }: ColorFieldProps) {
  return (
    <AriaColorField channel={channel} className={cn(colorFieldStyles(), className)} colorSpace={colorSpace}>
...
export default function ColorPicker({
  ...
  const [swatchPage, setSwatchPage] = useState(() => {
    const currentColorHex =
      (controlledValue as string | undefined)?.toLowerCase() ?? safeDefaultValue.toString('hex').toLowerCase();
