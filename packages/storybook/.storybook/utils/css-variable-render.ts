 ...
 */
export function readDefaultThemeValues(element: HTMLElement, theme: 'light' | 'dark'): ThemeValues | null {
  ...
  if (!testValue && process.env.NODE_ENV === 'development') {
    // eslint-disable-next-line no-console
    console.warn('CSS variables may not be loaded. Check that @packages/styles CSS files are imported.');
  }
