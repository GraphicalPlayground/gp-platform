 ...
 * Aligned with GP-Platform theme system - starts with defaults from variables.css and applies hue/chroma adjustments
 */
 ...
 */
function adjustHue(color: { l: number; c: number; h: number }, newHue: number): { l: number; c: number; h: number } {
...
export function generateThemeColors(hue: number, chroma: number): GeneratedColors {
  ...
  const createThemeColor = (
    ...
    return {
      ...
      oklchDark: darkColor ? formatOklch(darkColor) : typeof darkValue === 'string' ? darkValue : formatOklch(darkValue)
    };
  };
  ...
  const accentForeground: ThemeColor = createThemeColor('--accent-foreground', 'var(--snow)', 'var(--snow)');
  ...
  const surfaceSecondaryColor: ThemeColor = {
    ...
    oklchLight: formatOklch(adjustChroma(adjustHue(defaultsLight.surfaceSecondary, hue), chroma * 0.8)),
    oklchDark: formatOklch(adjustChroma(adjustHue(defaultsDark.surfaceSecondary, hue), chroma * 1.5))
  };
  ...
  const surfaceTertiaryColor: ThemeColor = {
    ...
    oklchLight: formatOklch(adjustChroma(adjustHue(defaultsLight.surfaceTertiary, hue), chroma * 0.8)),
    oklchDark: formatOklch(adjustChroma(adjustHue(defaultsDark.surfaceTertiary, hue), chroma * 1.5))
  };
...
export function buildThemeCssVars(colors: GeneratedColors, theme: 'light' | 'dark'): string {
  ...
  vars.push(`--color-default-hover: color-mix(in oklab, ${defaultValue} 90%, ${getValue(colors.foreground)} 10%)`);
  ...
  vars.push(`--color-danger-hover: color-mix(in oklab, ${dangerValue} 90%, ${getValue(colors.dangerForeground)} 10%)`);
